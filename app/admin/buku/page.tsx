  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingBook
        ? `/api/admin/buku/${editingBook.idBuku}`
        : '/api/admin/buku';
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row gap-4 justify-between items-center bg-slate-50/30">
          ...
          <button className="flex items-center gap-2 px-4 py-2.5 text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors font-medium text-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-500 font-semibold uppercase tracking-wider text-xs">
              <tr>
                <th className="px-8 py-5">Informasi Buku</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                filteredBooks.map((book) => (
                  <tr key={book.idBuku} className="hover:bg-slate-50/80 transition-colors group">
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-semibold text-slate-900 line-clamp-1 text-base">{book.judul}</p>
                          <p className="text-sm text-slate-500 mt-0.5">{book.pengarang}</p>
                        </div>
                      </div>
                    </td>
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-8">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        >
                          ...
                          {categories.map((c) => (
                            <option key={c.idKategori} value={c.idKategori}>{c.namaKategori}</option>
                          ))}
                        </select>
                      </div>
                ...
                <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl border border-slate-100">
                  ...
                  />
                  <div>
                    <label htmlFor="statusAktif" className="text-sm font-semibold text-slate-800 block">Status Aktif</label>
