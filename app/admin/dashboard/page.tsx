                        ...
                        {data.notifikasi.length > 0 ? (
                            data.notifikasi.map((notif, idx) => (
                                <div
                                    ...
                                    className="relative pl-6 pb-6 border-l border-slate-100 last:pb-0 last:border-0">
                                    <div
                                        className={`absolute left-[-5px] top-0 w-2.5 h-2.5 rounded-full ring-4 ring-white ${
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-400 mt-1.5">
                                            {new Date(notif.tanggalNotifikasi).toLocaleString("id-ID", {
                                                dateStyle: "medium",
                                                timeStyle: "short",
                                            })}
                                        </p>
                            ...
                            {data.pesananTerbaru.length > 0 ? (
                                data.pesananTerbaru.map((pesanan) => (
                                    <tr
                                        ...
                                        <td className="px-8 py-5 text-slate-900 font-bold">{formatCurrency(Number(pesanan.totalBayar))}</td>
                                        <td className="px-8 py-5">
...
function StatusBadge({ status }: { status: string }) {
    ...
    return (
        <span
            className={`px-3 py-1 rounded-full text-xs font-semibold border ${styles[status] || "bg-slate-50 text-slate-700 border-slate-200"}`}>
