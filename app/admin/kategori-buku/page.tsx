  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `/api/admin/kategori-buku?page=${currentPage}&limit=10&search=${encodeURIComponent(searchTerm)}`,
      );
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingCategory
        ? `/api/admin/kategori-buku/${editingCategory.idKategori}`
        : '/api/admin/kategori-buku';
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                categories.map((category) => (
                  <tr key={category.idKategori} className="hover:bg-slate-50/80 transition-colors group">
                    <td className="px-6 py-5 text-slate-500 font-mono text-xs">#{category.idKategori}</td>
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-3">
        ...
        {isModalOpen && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg m-4 sm:m-6 overflow-hidden max-h-[90vh] flex flex-col z-10"
            >
