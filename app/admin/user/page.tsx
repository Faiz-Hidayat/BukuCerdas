...
function UserContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = currentUser ? `/api/admin/user/${currentUser.idUser}` : '/api/admin/user';
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 bg-gradient-to-br from-slate-200 to-slate-300 rounded-full overflow-hidden relative shrink-0 shadow-inner ring-2 ring-white">
                            {user.fotoProfilUrl ? (
                              <Image src={user.fotoProfilUrl} alt={user.namaLengkap} fill className="object-cover" />
                            ) : (
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">
                    Password{' '}
                    {currentUser && (
                      <span className="text-slate-400 font-normal">(Kosongkan jika tidak ingin mengubah)</span>
                    )}
                  </label>
